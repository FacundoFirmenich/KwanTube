...
NC = 7
NK = 1
T_TARGET_PS = 10.0   # Initial target
WINDOW_PS   = 2.0
DT_FS       = 5.0
...
# Files — sincronizados con la raíz
CKPT_FILE = PROJECT_ROOT / "heom_1jff_prod_checkpoint.pkl"
TRAJ_FILE = PROJECT_ROOT / "heom_1jff_prod_trajectory.pkl"
LOG_FILE  = PROJECT_ROOT / "production_progress.log"
...
def load_1jff():
    ...
    if not npz_path.exists():
        log(f"ERROR: No se encuentra {npz_path}")
        sys.exit(1)
...
def main():
    ...
    for w in range(total_windows):
        ...
        # result.expect is list of lists (one per e_op)
        for i in range(N):
            # Skip the first element if it overlaps with previous window end
