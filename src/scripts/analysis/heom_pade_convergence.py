...
# --- Utilidades de Selección de Fragmentos ---
...
def run_converged_check():
    # 1. Cargar el Hamiltoniano real de 6DPU - Estrictamente desde raw_npz
    npz_path = PROJECT_ROOT / "outputs_data" / "raw_npz" / "H_6DPU.npz"
    if not npz_path.exists():
        raise FileNotFoundError(f"Error: No se encuentra H_6DPU.npz en {npz_path}")
    ...
    json_path = PROJECT_ROOT / "outputs_data" / "raw_json" / "metrics" / "heom_convergence_summary.json"
    ...
    txt_path = PROJECT_ROOT / "outputs_data" / "raw_txt+md" / "reports" / "heom_convergence_report.txt"
