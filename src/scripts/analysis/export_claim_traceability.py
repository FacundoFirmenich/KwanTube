...
def main() -> None:
    ...
    # Row counts
    n_structures = _count_rows(compact_dir / "structures_compact.csv")
    n_studies    = _count_rows(compact_dir / "studies_compact.csv")
    n_spectral   = _count_rows(compact_dir / "spectral_compact.csv")
    n_metrics    = _count_rows(compact_dir / "metrics_compact.csv")
    n_panels     = _count_rows(compact_dir / "comparative_panels_compact.csv")
    n_registry   = _count_rows(compact_dir / "data_registry.csv")
    n_fisher     = _count_rows(analysis_dir / "theory" / "fisher_barrier.csv")

    # Empirical bath parameters
    bath = _read_single_csv(bath_dir / "bath_params_empirical.csv")
    if bath:
    ...
    # Mechanism panels
    panels = _read_panels(compact_dir / "comparative_panels_compact.csv")
    panel_rows = ""
    for p in panels:
        panel_rows += (
            f"| {p.get('mechanism')} "
