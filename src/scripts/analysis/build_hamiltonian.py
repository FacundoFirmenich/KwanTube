...
def _resolve_project_root() -> Path:
    ...
    candidates = [
        Path(__file__).resolve().parents[3],   # src/scripts/data/ -> raíz
        Path(__file__).resolve().parents[2],   # fallback por si acaso
        Path.cwd(),
    ]
...
def build(
    ...
    assert herm_err < HERMITICITY_THRESHOLD, (
        f"H no es hermítico para {pid}: ||H - H†|| = {herm_err:.2e} "
    ...
    return {
        ...
        "physics_params_sha256": hashlib.sha256(
            json.dumps(PHYSICS_PARAMS, sort_keys=True, default=str).encode("utf-8")
        ).hexdigest(),
    }
...
def _sha256_file(path: Path) -> str:
    """Calcular SHA-256 de un archivo sin cargarlo completo en memoria."""
...
from qmc_mt.validate_integrity import write_sidecar

def save_npz(result: dict, out: Path) -> None:
    """Guardar Hamiltoniano en formato .npz con sidecar SHA-256."""
    np.savez(
        out,
        H_cm1=result["H_cm1"],
        ...
        mu_D=result["mu_D"],
        eps_r=result["eps_r"],
        physics_params_sha256=np.array(result["physics_params_sha256"]),
    )
    record = write_sidecar(out, overwrite=True)
    print(f"  guardado: {out} (status={record.status}, sha256={record.sha256[:16]}...)")


def verify_npz(path: Path) -> bool:
    """Verificar sidecar SHA-256 de un Hamiltoniano .npz."""

    sidecar = path.with_suffix(path.suffix + ".sha256")
    if not sidecar.exists():
        warnings.warn(f"No existe sidecar SHA-256 para {path}", stacklevel=2)
        return False
