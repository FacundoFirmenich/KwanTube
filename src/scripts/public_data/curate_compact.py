...
PROJECT_ROOT   = KWANTUBE_ROOT      # alias kept for downstream references
...
def _iter_latest_raw_json() -> Iterable[Path]:
    """Return the Path of the most recent timestamped snapshot folder.

    Searches first in the canonical companion-data directory
    (data_downloaded_public_repos/raw/public_api/) and falls back to the
    KwanTube-internal outputs_data/raw_data/public_api/ written by fetch_public_data.py.
    """
    for candidate_root in (
        ...
        if not candidate_root.exists():
            continue
        runs = sorted([p for p in candidate_root.iterdir() if p.is_dir()])
        if runs:
            return runs[-1]
    return []
