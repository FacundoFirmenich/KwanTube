...
def _write_progress(step: str, payload: Dict[str, Any]) -> None:
    ...
    progress_path = progress_dir / "_progress_fetch_public_data.json"
    blob = {
...
def main() -> None:
    ...
    # Write outputs to the canonical companion-data directory so that curate_compact.py
    # and build_registry.py can locate them without path re-configuration.
    _HERE          = Path(__file__).resolve()
    WORKSPACE_ROOT = _HERE.parents[4]   # …/biofisicaquantiqaCLINE/
    KWANTUBE_ROOT  = _HERE.parents[3]   # …/KwanTube/
    OUTPUTS_ROOT   = KWANTUBE_ROOT / "outputs_data"   # kept for progress/flags

    out_dir = WORKSPACE_ROOT / "data_downloaded_public_repos" / "raw" / "public_api" / ts
    ...
    done_flag = OUTPUTS_ROOT / "raw_json" / "progress" / "_done_fetch_public_data.flag"
