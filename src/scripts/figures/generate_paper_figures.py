...
PROJECT_ROOT = Path(__file__).resolve().parents[3] # back from src/scripts/figures/ to root
if str(PROJECT_ROOT / "src") not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT / "src"))

# Force non-interactive backend for servers/terminals
import matplotlib
matplotlib.use("Agg")
...
# Redirected to outputs_data/figures_final/
OUT = PROJECT_ROOT / "outputs_data" / "figures_final"
...
def figure1_landscape():
    ...
    ax.set_xscale('log')
    try:
        # Synced with raw_json
        rf_path = PROJECT_ROOT / "outputs_data" / "raw_json" / "metrics" / "redfield_summary.json"
...
def figure2_signatures():
    ...
    ax_a.set_xlabel('Detuning Delta (THz)'); ax_a.set_ylabel('Freq (THz)'); ax_a.legend()
    ...
    ax_b.set_xlabel('Wavelength (nm)'); ax_b.set_ylabel('d^2A/dlambda^2')
    ...
    ax_c.set_xlabel('SNR'); ax_c.set_ylabel('DeltaBIC (Decisive evidence > 10)')
...
def figure3_frohlich():
    ...
    ax_a.set_xlabel('N dimers'); ax_a.set_ylabel('eta pumping')
...
def figure4_scaling():
    ...
    ax_a.set_xlabel('L_MT (mum)'); ax_a.set_ylabel('Splitting (meV)'); ax_a.set_ylim(13.5, 14.5)

    eps = np.linspace(30, 120, 50)
    tau = [5e-7 * (e/80)**2 * 1e6 for e in eps]
    ax_b.plot(eps, tau, color=MAP['cavity'], lw=4)
    ax_b.set_xlabel('Dielectric epsilon'); ax_b.set_ylabel('tau_cavity (mus)')
